...
class GoTo:
	...
	async def __call__(self, drone):
		...
		try:
			...
			return
		print(f"-- Command: Position {self.target[0]}m North, {self.target[1]}m East, {self.target[2]}m")
		...
		async for position_ned in drone.conn.telemetry.position_velocity_ned():
			...
			if distance_between(self.target, position) < self.tolerance:
				print(f"-- Arrived @ Position {self.target[0]}m North, {self.target[1]}m East, {self.target[2]}m Down ")
				break
